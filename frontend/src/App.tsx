...
export default function App() {
  ...
  return (
    <div data-workspace={mode} className={`app-shell ${navigationCollapsed ? 'navigation-is-collapsed' : ''}`} ref={shellRef} style={layoutStyle}>
      ...
      {mode === 'quantjudge' ? <div className="qj-global-rail"><strong>PRIVACY MODEL</strong><span><b>私密</b>策略源码与 Agent 参数</span><span><b>私密</b>原始投资决策</span><span className="is-public"><b>公开</b>验算收益与证明回执</span></div> : mode === 'research' ? (market?.is_stale || integrity.source?.startsWith('demo') ? <div className="research-data-status" role="status">{integrity.source?.startsWith('demo') ? '当前使用演示数据，不能作为真实行情验证。' : '行情刷新失败，当前使用缓存数据。'}</div> : null) : <IntegrityRail dataSource={integrity.source} tradeCount={integrity.trades} hasResult={Boolean(activeResult)} warningCount={integrity.warnings} isStale={market?.is_stale} lastBarTime={market?.last_bar_time} />}
