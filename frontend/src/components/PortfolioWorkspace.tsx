  ...
  return (
    <div className="portfolio-workspace">
      <section className="portfolio-builder">
        ...
        {strategyId !== 'risk_parity' && <ul className="allocation-legend" aria-label="资产目标权重明细">{activeSelected.map((item, index) => <li key={item.asset.symbol}><i aria-hidden="true" style={{ background: `hsl(${160 + index * 42} 58% 52%)` }} /><span>{item.asset.symbol}</span><strong>{formatPercent(item.weight)}</strong></li>)}</ul>}
      </section>
