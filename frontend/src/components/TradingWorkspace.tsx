  ...
  return <section className="trade-run-detail" id={`detail-${run.id}`} aria-label={`${run.strategy_name} 运行详情`}>
    ...
    <div className="trade-detail-layout">
      ...
      <section><h5>当前资产</h5><dl className="trade-position-list">
        <div><dt>现金</dt><dd>{money(run.cash, runCurrency(run))}</dd></div><div><dt>持仓数量</dt><dd>{run.quantity}</dd></div>
...
function AccountConnection({ caps, venue, accounts, locked, onRefresh }: ConnectionProps) {
  ...
  return <>
    <div className="trade-connection-summary">
      ...
      <p className="trade-connection-hint">{!caps ? '读取完成后将显示配置状态与下一步。' : !caps.authorized ? '需先在服务端将当前平台用户绑定为交易账户所有者。' : !selected?.configured ? '服务端尚未配置完整的交易所凭据，请按指引完成设置。' : result ? '本次余额查询成功；刷新页面或切换账户后需重新验证。' : '凭据已配置。查询余额成功后，才能确认账户实际连通。'}</p>
      <div className="trade-connection-actions">
      ...
      <p className="trade-permission">交易权限：{!caps ? '待读取' : selected?.can_trade ? '已启用 · 仍需逐笔确认' : '提交未启用'}{ready && !selected?.can_trade && ' · 可验证连接'}</p>
    ...
    {guideOpen && <section className="trade-connection-guide" id="trade-connection-guide" aria-label="接入指引">
      ...
      <ol><li><strong>绑定当前平台用户</strong><p>将 <code>ATLAS_TRADING_OWNER_ID</code> 设置为当前用户 ID：</p>{caps?.user_id ? <code className="trade-user-id">{caps.user_id}</code> : <p>尚未读取到用户 ID，请重新检测配置。</p>}</li>
        <li><strong>配置 {names[venue]} 凭据</strong><p>在后端环境中设置 <code>{prefix}_API_KEY</code>、<code>{prefix}_API_SECRET</code>{venue === 'okx' && <> 和 <code>ATLAS_OKX_PASSPHRASE</code></>}。<code>{prefix}_MODE</code> 默认 <code>demo</code>（测试），实盘为 <code>live</code>，凭据必须与环境匹配。</p><p>密钥仅在服务端保存，不要填入前端、聊天或代码仓库；不需要提现权限。</p></li>
      ...
      <details className="trade-enable-help"><summary>需要人工下单？</summary><p><code>ATLAS_TRADING_ENABLED=1</code> 允许提交；实盘还需 <code>ATLAS_TRADING_LIVE_ENABLED=1</code>。连接成功不会自动开启交易，每笔订单仍需预览和确认。</p></details>
    </section>}
...
export default function TradingWorkspace() {
  ...
  const [composer, setComposer] = useState<'run' | 'order' | null>(query.has('release') ? 'run' : null)
  ...
  return <main className="trading-workspace trading-console">
    <header className="trade-heading">
      <div><h1>交易账户<span className="trade-title-dot" aria-hidden="true" /></h1><p>管理账户，让每一次决策有据可循。</p></div>
    ...
    <div className="trade-workbench">
      ...
      <div className="trade-main-content">
        <section hidden={view !== 'overview'} aria-label="账户概览">
          <div className="trade-overview-heading"><div><span className="trade-context-label">{selectedName} · 现货账户</span><h2>{!caps ? '正在读取账户' : !caps.authorized || !selected?.configured ? '从连接账户开始' : '查看账户，掌握资产状态'}</h2><p>账户资产与策略收益分开记录，测试环境与实盘始终清晰。</p></div><span className={`trade-overview-symbol is-${venue}`} aria-hidden="true"><Wallet size={48} strokeWidth={1.15} /></span></div>
          <div className="trade-overview-grid">
            <section className="trade-account" aria-label="账户与连接"><AccountConnection key={`${venue}:${caps?.authorized}:${selected?.configured}:${selected?.mode}`} caps={caps} venue={venue} accounts={accounts} locked={busy || !!preview} onRefresh={load} /></section>
    ...
    <Dialog.Root open={composer !== null} onOpenChange={(open) => { if (!open && !busy) { setComposer(null) } }}>
      <Dialog.Portal><Dialog.Backdrop className="trade-dialog-backdrop" /><Dialog.Popup className="trading-workspace trade-modal" aria-describedby="trade-composer-description">
        <header className="trade-modal-heading"><div><Dialog.Title>{composer === 'run' ? '新建策略运行' : '人工下单'}</Dialog.Title><Dialog.Description id="trade-composer-description">{composer === 'run' ? '选定策略与资金，开始持续记录。' : '核对账户与参数，再逐笔确认提交。'}</Dialog.Description></div><Dialog.Close disabled={busy} aria-label="关闭操作面板"><X size={20} /></Dialog.Close></header>
        {composer === 'run' ? <><div className="trade-run-form-note"><Layers3 size={18} /><p>每个实例只负责一个标的；平台模拟不使用真实资金。</p></div>      {!allowedReleases.length && <p className="trade-composer-empty">还没有可运行策略。<a href="#quantjudge" onClick={() => setComposer(null)}>前往策略库发布或订阅</a>一个版本，再回来创建运行。</p>}
      <form className="trade-run-form" onSubmit={(event) => { event.preventDefault(); void action(async () => {
        ...
        const created = await request<StrategyRun>('/trading/runs', { method: 'POST', body: JSON.stringify({ release_id: release.id, subscription_id: subscription?.id, account_id: account?.id, market: runMarket, environment: runEnvironment, symbol: runSymbol, interval: runInterval, initial_cash: runCash, commission_rate: commissionRate, slippage_rate: slippageRate, max_position: maxPosition, max_participation: maxParticipation, stop_loss: stopLoss, take_profit: takeProfit }) })
        await request(`/trading/runs/${created.id}/start`, { method: 'POST' }); setNotice('策略实例已启动，首次运行将建立行情基线。'); await load(); setComposer(null); setView('strategies')
      }) }}>
        ...
        <button className="trade-primary" disabled={busy || !runRelease || !availableRunAccounts.length}><Play size={15} />{runEnvironment === 'platform_sim' ? '启动模拟' : '启动信号监控'}</button>
      ...
      {!selected?.can_trade && <p className="trade-ticket-hint">{!caps ? '正在读取交易权限…' : !caps.authorized || !selected?.configured ? '请先在账户概览完成配置，再检查交易权限。' : '交易提交未启用；验证连接仍可查询余额。'}策略信号始终需要人工确认。</p>}</section></>}
