...
export function ParameterSweep({ fields, payload, onApply, disabled = false }: { fields: SweepField[]; payload: Record<string, unknown>; onApply: (values: Record<string, number>) => void; disabled?: boolean }) {
  ...
  async function run() {
    ...
    if (validation || !start || !end || !Number.isFinite(Date.parse(start)) || !Number.isFinite(Date.parse(end)) || Date.parse(start) >= Date.parse(end)) { setStatus(validation || '请选择有效的历史开始和结束时间'); return }
    const abort = new AbortController(); controller.current = abort; setBusy(true); setResults([]); setErrors([]); setProgress(0)
    setSnapshot(identity); setPeriod(`${start.replace('T', ' ')} — ${end.replace('T', ' ')}（本地时间）`)
    ...
    try {
      for (const values of grid) {
        ...
        const body = { ...candidate, start: new Date(start).toISOString(), end: new Date(end).toISOString(), persist: false }
        try {
  ...
  return <details className="parameter-sweep"><summary>参数穷举 · 寻找历史收益最高组合</summary>
    <p>在指定范围逐个回测，按总收益率排序（最多 100 组）。沿用当前资金、费用与风控；批量搜索不运行 AI 流程。历史最优仅针对本次区间与网格，不代表未来收益。</p>
    <fieldset disabled={busy || disabled}><div className="sweep-dates"><label>开始时间<input type="datetime-local" value={start} onChange={e => setStart(e.target.value)} /></label><label>结束时间<input type="datetime-local" value={end} onChange={e => setEnd(e.target.value)} /></label></div>
    ...
    <small>{validation || `共 ${combinations.length} 组`}</small><button type="button" disabled={!!validation || !start || !end} onClick={() => void run()}>开始参数穷举</button></fieldset>
