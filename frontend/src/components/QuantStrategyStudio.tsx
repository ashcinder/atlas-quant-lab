...
export function QuantStrategyStudio({
  ...
  return <section className={`qjs-shell ${embedded ? 'is-embedded' : ''}`}>
    ...
    <header className="qjs-toolbar guided-toolbar">
      ...
      <details className="guided-save-settings" open={tab !== 'workflow' || undefined}><summary title="保存工作流或设置私密账户"><Save size={14} />保存</summary><form className="qjs-auth" onSubmit={(event) => { event.preventDefault(); void save() }}>
        ...
        <label className="qjs-agent-select"><span>策略身份</span><select name="agent-id" autoComplete="off" value={agentId} onChange={(event) => changeAgent(event.target.value)} aria-label="选择用于归属工作流的策略身份"><option value="">{agents.length ? '请选择' : '尚未创建策略身份'}</option>{agents.map((agent) => <option key={agent.id} value={agent.id}>{agent.name}</option>)}</select><ChevronDown size={12} aria-hidden="true" /></label>
