import { ChainOrderPanel } from './BkcControls'
  ...
  return (
    <section className={`results-panel panel-${panelMode}`} aria-label="回测结果">
      ...
      <div className="result-content" role="tabpanel">
        {!loading && result ? <div><p className="result-identity">当前结果：{result.strategy.name} · {result.data_source}{result.equity.length > 0 && ` · ${new Date(result.equity[0].time * 1000).toLocaleString()} — ${new Date(result.equity[result.equity.length - 1].time * 1000).toLocaleString()}`}</p><ChainOrderPanel key={result.run_id} kind="report" resourceId={result.run_id} /></div> : null}
