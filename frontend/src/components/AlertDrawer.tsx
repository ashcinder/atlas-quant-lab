...
export function AlertDrawer({ open, asset, interval, source, onClose, onUnread, onError }: Props) {
  ...
  return <div className={`drawer-backdrop ${open ? 'is-open' : ''}`} onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }}>
    <aside className="alert-drawer" aria-label="提醒中心" aria-hidden={!open}>
      <div className="drawer-heading"><span><BellRing size={15} />提醒中心</span><button className="icon-button" onClick={onClose}><X size={15} /></button></div>
      ...
      <div className="alert-scroll">
        <section className="alert-rule-list">{alerts.map((rule) => <div className={`alert-rule ${rule.enabled ? '' : 'is-disabled'}`} key={rule.id}><button className="alert-switch" aria-label={rule.enabled ? '停用提醒' : '启用提醒'} onClick={() => void toggle(rule)}><i /></button><span><strong>{rule.name}</strong><small>{rule.symbol} · {rule.interval} · {ALERT_KINDS.find((item) => item.value === rule.kind)?.label}</small><em>上次触发 {dateTime(rule.last_triggered_at)}</em></span><button className="rule-delete" onClick={async () => { await api.deleteAlert(rule.id); await load() }}><Trash2 size={12} /></button></div>)}</section>
