...
import { Boxes, Check, Play, RefreshCw, Rocket, X } from 'lucide-react'
...
import PrivateStrategyRegistration from './PrivateStrategyRegistration'

type View = 'discover' | 'mine' | 'subscriptions'
const marketNames: Record<RuntimeMarket, string> = { CRYPTO: '加密货币', US: '美股', CN: 'A 股' }

export default function StrategyRuntimeLibrary({initialView='discover'}:{initialView?:View}) {
  ...
  const visible = useMemo(() => releases.filter((item) => view === 'mine' ? item.owned : view === 'discover' ? item.published : false), [releases, view])
  ...
  return <section className="runtime-library">
    <header><div><em>策略库</em><h2>选择策略，订阅运行</h2><p>发布版本一经创建不可修改；订阅统一以 BKC 支付，授权锁定到具体版本。</p></div><button disabled={busy} onClick={() => void act(load)}><RefreshCw size={15} />刷新</button></header>
    <details className="runtime-tools"><summary>测试与高级工具</summary><div className="runtime-tool-actions"><button disabled={busy} onClick={() => void act(async () => {
    ...
    <button disabled={busy} onClick={() => void act(async () => {
      const example = await request<StrategyRelease>('/strategy-releases/quote-probe', { method: 'POST' })
      await request('/strategy-subscriptions', { method: 'POST', body: JSON.stringify({ release_id: example.id }) })
      window.location.hash = `#/trading?release=${example.id}`
    })}>订阅10秒买卖联调策略</button>
    </div><PrivateStrategyRegistration onCreated={load} /></details>
    ...
    {view === 'mine' && <form className="runtime-publisher" onSubmit={(event) => { event.preventDefault(); void act(publish) }}>
      ...
      <label><input type="checkbox" checked={publishPublicly} onChange={(event) => setPublishPublicly(event.target.checked)} />同时公开（未验证，设置 BKC 价格后可订阅）</label>
      <button className="runtime-primary" disabled={busy || !selected || !markets.length}><Rocket size={15} />保存不可变版本</button>
    </form>}
    {view === 'subscriptions' ? <div className="runtime-release-grid">{subscriptions.map((item) => <article key={item.id}><span className="runtime-version">v{item.version}</span><h3>{item.name}</h3><p>{item.markets.map((market) => marketNames[market]).join(' · ')}</p><small>版本指纹 {item.content_hash.slice(0, 10)}…</small>{item.upgrade_release_id && <p>作者已发布 v{item.upgrade_version}，当前运行实例仍锁定 v{item.version}。</p>}<footer><em className={item.status === 'active' ? 'is-active' : ''}>{item.status === 'active' ? '运行授权有效' : '已取消'}</em>{item.status === 'active' && <><a href={`#/trading?release=${item.release_id}&subscription=${item.id}`}><Play size={14} />去运行</a>{item.upgrade_release_id && <button onClick={() => setView('discover')}>查看 v{item.upgrade_version} 订阅价格</button>}<button aria-label={`取消订阅 ${item.name}`} onClick={() => void act(() => request(`/strategy-subscriptions/${item.id}`, { method: 'DELETE' }))}><X size={14} /></button></>}</footer></article>)}{!subscriptions.length && <p className="runtime-empty">还没有可运行订阅。</p>}</div> : <div className="runtime-release-grid">{visible.map((item) => <article key={item.id}><span className="runtime-version">v{item.version}</span><h3>{item.name}</h3><p>{item.description || '规则策略版本'}</p><small>{item.markets.map((market) => marketNames[market]).join(' · ')} · {item.source_kind === 'private_runner' ? '私有执行 · 源码不上传' : item.source_kind === 'python' ? '受限Python' : item.source_kind === 'custom' ? '自定义规则' : '内置规则'}</small><CloudProofControl releaseId={item.id} owned={item.owned} proofStatus={item.proof_status} proofId={item.proof_id} sourceKind={item.source_kind}/><StrategyAnchorPanel releaseId={item.id} owned={item.owned} /><BkcSubscription releaseId={item.id} owned={item.owned} subscribed={subscriptions.some(sub => sub.release_id === item.id && sub.status === 'active')} onChanged={load} /><footer><code>{item.content_hash.slice(0, 10)}…</code>{item.owned ? <><a href={`#/trading?release=${item.id}`}><Play size={14} />运行</a>{!item.published && <button disabled={busy} onClick={() => void act(() => request(`/strategy-releases/${item.id}/publish`, { method: 'POST' }))}>公开此版本</button>}</> : null}</footer></article>)}{!visible.length && <p className="runtime-empty">暂无公开版本。请在“我的策略”中公开已有版本，或前往策略实验室创建。</p>}</div>}
  </section>
}
