...
import { applySweep, numericFields } from '../parameterSweep'
...
export const ResearchWorkspace = memo(function ResearchWorkspace(props: ResearchWorkspaceProps) {
  ...
  const sweepBase = { symbol: props.asset?.symbol, asset_class: props.asset?.asset_class, interval: props.interval, data_source: props.source, initial_capital: props.initialCapital, commission_rate: props.commission, slippage_rate: props.slippage, spread_rate: props.spread, max_position: props.maxPosition, max_participation_rate: props.maxParticipation }
  return <main className={`research-workspace ${props.showHeader === false ? 'is-embedded' : ''}`}>
    ...
    {tab === 'optimize' ? <div className="research-layout">
      <aside className="research-config">
        ...
        {activeGridStrategy && <ParameterSweep key={activeGridStrategy.id} disabled={!props.asset || !!(job && ['queued', 'running'].includes(job.status))} fields={activeGridStrategy.parameters.filter(p => p.kind === 'number' || p.kind === 'integer').map(p => ({ key: `params.${p.key}`, label: p.label, value: Number(p.default), min: p.minimum ?? undefined, max: p.maximum ?? undefined, integer: p.kind === 'integer' }))} payload={{ ...sweepBase, strategy_id: activeGridStrategy.id, params: strategyDefaults(activeGridStrategy) }} onApply={values => { setGridText(Object.fromEntries(Object.entries(values).map(([key, value]) => [key.replace(/^params\./, ''), String(value)]))); setSelected(new Set([activeGridStrategy.id])); setObjective('total_return') }} />}
      ...
      <section className="builder-canvas">
        ...
        <ParameterSweep disabled={!props.asset || saving || builderRunning} fields={numericFields({ custom_strategy: buildSpec() })} payload={{ ...sweepBase, strategy_id: buildSpec().id, params: {}, custom_strategy: buildSpec() }} onApply={values => { const next = applySweep({ custom_strategy: buildSpec() }, values).custom_strategy; markBuilderEdited(); setTargetPosition(next.target_position); setEntryRules(ruleToConditions(next.entry)); setExitRules(ruleToConditions(next.exit)); setBuilderNotice({ tone: 'success', text: '已将搜索参数应用到图形化草稿，保存后生效。' }) }} />
