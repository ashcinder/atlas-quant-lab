...
1. 登录平台，打开「交易账户」，点击「查看接入指引」，取得当前用户 ID；设置 `ATLAS_TRADING_OWNER_ID`。
2. 设置下表中所需交易所的密钥。测试密钥与实盘密钥不可混用。币安当前支持 HMAC 密钥；欧易还需 Passphrase。密钥应仅有账户查询与现货交易权限，不需要提现权限。
3. 重启后端，在接入指引中点击「重新检测配置」，再点击「验证连接」查询余额。查询不需要开启交易提交；只有查询成功才说明账户实际连通，“凭据已配置”不代表连通。如需人工下单，再在服务端设置 `ATLAS_TRADING_ENABLED=1` 并重启，实盘另需下述开关。
